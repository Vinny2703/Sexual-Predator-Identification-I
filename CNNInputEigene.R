...
while(f <= xmlSize(xpathApply(tc, ("/conversations/conversation"), xmlAttrs))){
  text <- xpathApply(tc1, paste0("/conversations/conversation[position()='",f,"']/message/text"), xmlValue)
...
#Wörter in Wortliste
while(f < length(xpathApply(tc1, ("/conversations/conversation"), xmlAttrs))+1){
  text <- toString(xpathApply(tc1, paste0("/conversations/conversation[position()='",f,"']/message/text"), xmlValue))
